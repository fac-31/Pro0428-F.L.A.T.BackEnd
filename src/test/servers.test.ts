...
// Python server tests are more complex because it took longer to get the server running - can streamline later if needed
  ...
  try {
    ...
    if (axios.isAxiosError(error)) {
      ...
      if (error.code === 'ECONNREFUSED') {
        console.error(
          '\nConnection Refused: The Python server is not running or not accessible at',
          PYTHON_API_URL
        );
        console.error('Please ensure:');
        console.error('1. The Python server is started (npm run dev:python)');
        console.error('2. The server is running on port 8001');
